...
class ConfusionMatrix:
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            fig.savefig(Path(save_dir) / 'confusion_matrix.png', dpi=800)
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    ax.plot(px, py.mean(1), linewidth=1.5, color='blue', label='All classes %.3f mAP@0.5' % ap[:, 0].mean())
    ax.set_xlabel('Recall')
    ax.set_ylabel('Precision')
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left")
    fig.savefig(Path(save_dir), dpi=800)
...
def plot_mc_curve(px, py, save_dir='mc_curve.png', names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    y = py.mean(0)
    ax.plot(px, y, linewidth=1.5, color='blue', label=f'all classes {y.max():.2f} at {px[y.argmax()]:.3f}')
