...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.adan:
        #optimizer = Adan(params, lr = 1e-3, weight_decay = 0.02, betas = (0.02, 0.08, 0.01))
        optimizer = Adan(params=params, lr=hyp['lr'], weight_decay=opt.weight_decay, betas=opt.opt_betas, eps = opt.opt_eps, max_grad_norm=opt.max_grad_norm, no_prox=opt.no_prox)
        #optimizer = optim.Adamax(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
        optimizer = Adan(params, lr = 1e-3, weight_decay = 0.02, betas = (0.02, 0.08, 0.01))
        #optimizer = optim.Adamax(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adan', action='store_true', help='use torch.optim.Adan() optimizer')
